...
class AlipayServiceProvider extends ServiceProvider
{
	...
	protected function setupConfig()
	{
		...
		if ($this->app instanceof LaravelApplication && $this->app->runningInConsole()) {
			...
			$this->app->configure('alipay-mobile');
			$this->app->configure('alipay-web');
		}
		
		$this->mergeConfigFrom($source_config, 'alipay');
		$this->mergeConfigFrom($source_mobile, 'alipay-mobile');
		$this->mergeConfigFrom($source_web, 'alipay-web');
	}
	...
	public function register()
	{
		...
		$this->app->bind('alipay.mobile', function ($app)
		{
			...
			$alipay->setPartner($app->config->get('alipay.partner_id'))
				...
				->setPrivateKeyPath($app->config->get('alipay-mobile.private_key_path'))
				->setPublicKeyPath($app->config->get('alipay-mobile.public_key_path'))
		...
		$this->app->bind('alipay.web', function ($app)
		{
			...
			$alipay->setPartner($app->config->get('alipay.partner_id'))
				->setSellerId($app->config->get('alipay.seller_id'))
				->setKey($app->config->get('alipay-web.key'))
